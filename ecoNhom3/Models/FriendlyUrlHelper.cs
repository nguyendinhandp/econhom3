...
namespace ecoNhom3.Models
{
    public static class FriendlyUrlHelper
    {
        ...
        private static string RemapInternationalCharToAscii(char character)
        {
            ...
            if ("áàảạãăắằẳẵặâấầẩẫậ".Contains(s))
            {
                return "a";
            }
            else if ("êếềệểéèẹẻễẽ".Contains(s))
            {
                return "e";
            }
            else if ("íìỉịĩ".Contains(s))
            {
                return "i";
            }
            else if ("ôồốộổơờớợởòóọỏỗỗõ".Contains(s))
            {
                return "o";
            }
            else if ("ưứừựửủụúùữũ".Contains(s))
            {
                return "u";
            }
